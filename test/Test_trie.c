...
void test_trie_loop(void) {
	...
	while((loop_ptr = trie_loop_branch(root, loop_ptr, trie_filter_branch_end))) {
...
void test_filter_branch_simple(void) {
    ...
    TRIE_LOOP_INIT(&loop);
	char buf[1024] = "";
	while((loop_ptr = trie_filter_branch(root, loop_ptr, trie_filter_branch_end, pf))) {
...
void test_filter_accompanying_nodes(void) {
	...
	struct path_filter *pf = compile_filter_from_s("programming:langs:c:a");
	TRIE_LOOP_INIT(&loop);
	while((loop_ptr = trie_filter_branch(root, loop_ptr, trie_filter_has_data, pf))) {
        counter++;
	}
